...
LinkedIn Data Sync Extension TODOs

  Phase 1: Database Setup & Repository (Prompt 1)
[ ] Prisma Schema Update

[ ] Add LinkedinActivities model with unique index on linkedinPostId.

[ ] Add LinkedinCertifications model.

[ ] Define the mediaType Enum (Image, Video, Carousel, ExternalLink).

[ ] Database Migration

[ ] Run npx prisma migrate dev --name add_linkedin_sync_tables to update the PostgreSQL instance on Render.

[ ] Verify the tables generated cleanly in your database management tool.

[ ] Repository Implementation

[ ] Create src/repositories/linkedin.repository.ts.

[ ] Implement findActivityByLinkedinId, createActivity, and updateActivityMedia.

[ ] Implement findCertificationByCompositeKey and createCertification.

[ ] Testing

[ ] Create tests/linkedin.repository.test.ts.

[ ] Write tests confirming CRUD operations, unique constraints, and database lookups.

[ ] Execute tests using npm test and ensure 100% pass rate.

Phase 2: Media Ingestion Asset Pipeline (Prompt 2)
[ ] Configuration

[ ] Ensure CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET are added to your local .env and Render dashboard.

[ ] Utility Implementation

[ ] Create src/utils/media-processor.ts.

[ ] Implement syncMediaToCloudinary using Axios (arraybuffer) and Cloudinary’s uploader.upload_stream.

[ ] Implement the strict 10-second timeout logic block per asset.

[ ] Catch failures gracefully so a single corrupted image doesn't crash the whole loop.

[ ] Testing

[ ] Create tests/media-processor.test.ts.

[ ] Mock Axios and Cloudinary SDK responses.

[ ] Write unit tests verifying successful stream uploads and elegant timeout handling.

Phase 3: Intelligent Merge Core Engine (Prompt 3)
[ ] Core Service Implementation

[ ] Create src/services/sync-engine.service.ts.

[ ] Implement syncLinkedInData(incomingData).

[ ] Inject linkedin.repository.ts and media-processor.ts into the service.

[ ] Business Logic Enforcement

[ ] Certifications Check: Match using title + issuingBody as a composite key. Create if new; skip entirely if it already exists.

[ ] Featured Posts Check: Match using linkedinPostId. Create if new; if it exists, update only the mediaUrls and preserve your custom textContent.

[ ] Testing

[ ] Create tests/sync-engine.test.ts.

[ ] Test scenario: New items add cleanly.

[ ] Test scenario: Existing certifications are skipped safely.

[ ] Test scenario: Existing posts update media fields but leave edited copy intact.

Phase 4: Render Express Endpoints & Security (Prompt 4)
[ ] Security Middleware

[ ] Create src/middleware/auth.middleware.ts.

[ ] Implement token check comparing X-Sync-Token header against process.env.RENDER_SYNC_TOKEN.

[ ] Routing & Controllers

[ ] Create src/routes/sync.routes.ts.

[ ] Implement POST /api/sync-linkedin route guarded by the auth middleware.

[ ] Wire the controller to hand incoming payloads over to syncLinkedInData.

[ ] Build global try/catch wrapper yielding explicit 500 Internal Error responses on downstream failures.

[ ] Testing & Verification

[ ] Create tests/sync.routes.test.ts.

[ ] Use supertest to confirm bad/missing headers return a 401 Unauthorized status.

[ ] Confirm valid handshake requests trigger processing and respond with a 200 OK status.

Phase 5: Next.js Vercel Edge Cron Proxy (Prompt 5)
[ ] Vercel Infrastructure Setup

[ ] Create or update vercel.json in your Next.js project root.

[ ] Append the weekly cron object matching the path /api/cron/sync-linkedin.

[ ] Route Handler Implementation

[ ] Create app/api/cron/sync-linkedin/route.ts using Next.js App Router rules.

[ ] Add incoming header parsing comparing Authorization against Bearer ${process.env.CRON_SECRET_TOKEN}.

[ ] Write the outbound fetch() logic passing X-Sync-Token securely to the Render backend service.

[ ] Testing

[ ] Validate header rejection states using local curl scripts or Postman.

[ ] Confirm proxy forwarding correctly translates statuses from Render back to Vercel.

Phase 6: Next.js Cache Revalidation endpoint (Prompt 6)
[ ] Cache Buster Implementation

[ ] Create app/api/revalidate/route.ts.

[ ] Implement security validation layer matching Bearer ${process.env.REVALIDATION_SECRET_TOKEN}.

[ ] Parse JSON payload model fields (activities vs. certifications).

[ ] Integrate Next.js native revalidateTag('linkedin-activities') or revalidateTag('linkedin-certs').

[ ] Integration Validation

[ ] Test endpoint using mock fetch triggers to confirm the edge cache updates instantly without requiring a full platform redeploy.

Phase 7: Portfolio Presentation UI Layer (Prompt 7)
[ ] Data Fetching Implementation

[ ] Build the tagged backend fetch integration inside your server components: fetch(..., { next: { tags: ['linkedin-certs'] } }).

[ ] Time Expiry Filter: Inject logic filtering out elements where the certification expiryDate is less than new Date().

[ ] Tailwind CSS Frontend Components

[ ] Build UI display cards optimized for text, dynamic images, and HTML5 responsive <video> blocks.

[ ] Enforce security requirements: ensure all target anchors utilize target="_blank" and rel="noopener noreferrer".

[ ] Ensure components handle empty/missing linkPreviewCard attributes gracefully without throwing runtime layout bugs.

[ ] Testing

[ ] Create frontend unit tests using React Testing Library.

[ ] Assert that expired milestones do not appear on screen.

[ ] Verify presence of security parameters (target="_blank", rel="noopener noreferrer") across all rendering elements.